...
export default {
  ...
  methods: {
    initMap() {
      ...
      // var point = new BMapGL.Point(119.95629, 30.275156); // 创建点坐标
      var point = new BMapGL.Point(112.257777, 37.295026); // 创建点坐标
      ...
      this.map.setTilt(70); //0-90//地图纬度转动角度，0：从上至下，90：平行于地面
      // **********************************************************************************//
      this.roadLayer();
      this.carLayer();
    },
    roadLayer() {
      ...
      lineLayer.setData(this.data);
    },
    carLayer() {
      this.carlineLayer = new mapvgl.CarLineLayer({
        url: '/static/car/scene.gltf',
        autoPlay: true,
        scale: 5,
        step: 0.2
      });
      this.view.addLayer(this.carlineLayer);
    ...
    updateCar() {
      ...
      setInterval(() => {
        ...
        if (data[0].geometry.coordinates.length === 1) {
          index += 1;
          data[0].geometry.coordinates.push(this.coordinates[index]);
          console.log(data);
          ...
          var distance = this.map.getDistance(
            new BMapGL.Point(...data[0].geometry.coordinates[0]),
            new BMapGL.Point(...data[0].geometry.coordinates[1])
          );
          console.log(distance);
          console.log(this.carlineLayer);
          this.carlineLayer.options.step = distance;
          this.carlineLayer.setData(data);
        } else {
          index += 2;
          data[0].geometry.coordinates.push(this.coordinates[index]);
          data[0].geometry.coordinates.shift();
          console.log(data);
          var layer = new mapvgl.PointLayer({
            color: 'red',
            shape: 'circle', // 默认为圆形，可传square改为正方形
