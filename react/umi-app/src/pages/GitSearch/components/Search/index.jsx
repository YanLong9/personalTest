...
export default class Search extends Component {
	...
	search = () => {
		...
		//发送请求前通知List更新状态
		PubSub.publish("searchResult", { isFirst: false, isLoading: true, err: '' });
		//发送网络请求
		//#region axios
		...
		(async () => {
			try {
				const response = await fetch(`https://api.github.com/search/users?q=${keyWord}`)//连接服务器成功
				const data = await response.json()//获取接口返回的数据
				console.log(data);
